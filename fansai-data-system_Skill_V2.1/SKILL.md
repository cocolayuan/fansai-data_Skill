...
description: >
  ...
  账号管理/客户资料/客户库/线索池/客户节奏工作台/客户跟踪/AI 生成弹窗, a dashboard, a list page, a detail page, a KPI card row, or a data
  ...
  V2.1 inherits V2: 数据诚实铁律, 图表选型决策表, 列表页规范, 单层背景, 等高面板,
  语义色注册表 and interaction hygiene. V2.1 adds 全系统主记录—子记录层级, 默认/审计双视图,
  局部时间轴滚动, 固定操作列, 分组菜单 and 统一业务弹窗；客户库/线索池是首个完整业务映射. It also nails the classics: page margins (左右 36px), 字重(Noto Sans SC 用 500),
  面包屑式 header, 多个数字同屏布局, 表单与弹窗(自定义瓦片下拉替代原生 select), 图标(IconPark 内联 SVG),
  头像(hash 确定性分配), and HTML→SVG→Figma delivery rules.
---

# FansAI 数据中台 · 页面系统规范 V2.1
...
> - `references/system-workbench-patterns.md` — 全系统工作台、主从层级、双视图、局部时间轴、固定操作区与弹窗家族 🆕
> - `references/customer_workbench_reference.html` — 上述全系统规则的客户资料完整交互范例 🆕
>
> **完整继承 V2 与 V1.3 基线并收进 V2.1 包**（V2.1 可独立安装使用）
...
V2.1 在 V2 完整基线上新增全系统工作台与复合数据页规则。下表 1–9 为 V2 基线，10 为本次 V2.1 增量。
...
| 10 | **全系统工作台与复合数据页**（§7A 新增） | 主从实体层级、默认/审计双视图、仅时间窗口局部滑动、固定操作列、分组菜单与统一弹窗家族；客户资料为首个完整范例 |

沿用 V1.3 全部内容：设计令牌、字重铁律、36px 边距、面包屑 header、数字 6 模式、表单与弹窗、IconPark 内联 SVG、头像 hash 分配、Figma 交付规则。

### 继承与覆盖规则（重要）

- **V2.1 = V2 完整基线 + 全系统工作台与复合数据页增量**，不依赖另装 V2 或 V1.3。V2.1 未重述的细节继续按 V2 与 `references/v1.3-baseline.md` 执行。
...
## 7A. 全系统工作台与复合数据页（V2.1 新增）

完整规范与验收清单见 `references/system-workbench-patterns.md`，客户资料可运行范例见 `references/customer_workbench_reference.html`。

- 所有模块先按数据关系选择概览、平级列表、主从层级或时间节奏页型，不无差别复制客户页面模块。
- 主记录是父行、子记录是子行；同一父级下的子项允许不同阶段、审批和下一步。展开槽和细圆头虚线必须跨行对齐。
- 1440×900 优先，搜索收窄，筛选/双视图/主按钮尽量一行；同组运营数字合并为一个玻璃面板。
- 存在连续时间数据时，只有时间窗口横向滑动，表头与行同步；主记录信息和右侧操作固定，表头线两端给左右滑动箭头。
- 审计表格视图用于完整字段核对，可内部横向滚动；首列 sticky 且每种行状态都必须有不透明背景。
- 阶段用普通文字 + 四段条，不用 tag；`已推进完成`、`已排期`、`审批流程进行中`遵循同一字号和正文色。
- 右侧更多菜单按已发生/当前/接下来分组，使用统一 IconPark 线性图标；位置可上下翻转且不能被 overflow 裁切。
- 新建、编辑、审批、安排等短流程全部复用同一居中弹窗家族；自定义下拉和日历必须留箭头安全区并适配视口。
- 客户库/线索池、客户—项目是业务映射范例，仅在对应数据存在时使用，不能被误当作全系统固定字段。
